...
fun SignConfig.toSignConfigBean(path: String): SignConfigBean {
    return SignConfigBean(signingPath = "$path/$signFileName", alias = alias, passWord = passWord, signVersion = signVersion,keyPassWord=keyPassWord)
}
...
fun getSignConfigBean(signConfigPath:String):SignConfigBean{
    ...
    val gson = Gson()
    val fromJson = gson.fromJson(readText, SignConfig::class.java) ?: throw RuntimeException("$signConfigPath  is not SignConfig.json")
