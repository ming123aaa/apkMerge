...
fun getChannelConfigBean(channelConfigFile: File): ChannelConfigBean {
    ...
    val gson = Gson()
    val channelConfig = gson.fromJson(readText, ChannelConfig::class.java)
        ?: throw RuntimeException("$channelConfigFile  is not ChannelConfig.json")
...
fun List<ChannelConfigItem>.toBean(rootPath: String): List<ChannelConfigItemBean> {
    return filter { it.channelName.isNotEmpty() }.map { it.toBean(rootPath) }
}
