...
public class CommandArgs {
    ...
    @Parameter(names = {"-baseApk"}, description = "主包路径")
    public String baseApk = "";

    @Parameter(names = {"-channelApk"}, description = "渠道框架包路径")
    public String channelApk = "";
    ...
    @Parameter(names = {"-mergeSmali"}, description = "合并smali环境 需要配合-baseSmali和-channelSmali一起使用" +
    ...
    @Parameter(names = {"-generateMultipleChannelApk"},description = "批量生成渠道apk 需要配合命令-baseApk、-channelConfig" +
            "、-libs一起使用")
    ...
    @Parameter(names = {"-test"}, description = "测试运行")
    public boolean isTest = false;

    @Parameter(names = {"-outApkConfig"}, description = "输出ApkConfig.json的模板  需要使用-out设置输出路径的文件夹")
    ...
    @Parameter(names = "-sign",description = "给apk签名,配合-libs,-out、-signConfig和-baseApk一起使用")
    public boolean isSign=false;

    @Parameter(names = "-toApk",description = "smali环境生成apk,配合-libs,-out、-signConfig和-baseSmali一起使用")
    public boolean isToApk =false;

    @Parameter(names = "-outSignConfig", description = "输出signConfig.json的模板  需要使用-out设置输出路径的文件夹")
    public boolean isOutSignConfig = false;

    @Parameter(names = {"-outChannelConfig"},description = "输出ChannelConfig.json的模板  需要使用-out设置输出路径的文件夹")
    public boolean isOUtChannelConfig=false;

    @Parameter(names = {"-changeSmali"}, description = "需要修改的smali路径，配合-ApkConfig一起使用 可用于换包名、图标、名称")
    public String changeSmali = "";

    @Parameter(names = {"-changeApk"}, description = "需要修改apk的路径,配合-ApkConfig、-out、-signConfig、libs一起使用 可用于换包名、图标、名称")
    public String changeApk = "";


    @Parameter(names = {"-decompile"},description = "反编译apk ,配合-baseApk、-out、libs一起使用")
